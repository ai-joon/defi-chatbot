...
class SearchSchema(BaseModel):
    ...
    influence_filter: List[Literal["bot", "organic"]] = Field(
        [],
        description="An optional array of filter terms for filtering whether the post comes from a real person on social media (organic), or from fake accounts on social media (bot) (Must be an array!).",
    )
...
class KoatSocialsTool(BaseTool):
    name = "social_media_expert"
    description = """Useful for when you need to answer questions about social media content. In order to set the timeframe, you must use the 'from_date' and 'to_date' parameters. If asked about a date range relative the current date, use "now".
    """
                ...
                json.dumps(
                    {
                        ...
                        "message": "Using KOAT Socials tool...",
                    }
                ),
                request_context.message_queue,
            )

        score = agent_config["socials_tool"]["score_override"]

        if influence_filter == "media":
            size = agent_config["socials_tool"]["media_num_results"]
        else:
            size = agent_config["socials_tool"]["socials_num_results"]
