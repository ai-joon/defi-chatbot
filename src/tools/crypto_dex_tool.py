...
tools = load_tools(
    "ta_config/indicators", "ta_config/crypto_dex_config.yaml", "CRYPTO", "DEX"
)
...
class SearchSchema(BaseModel):
    question: str = Field(
        description="Forward the user's question to the crypto dex expert"
    )


class CryptoDexTool(BaseTool):
    name = "crypto_dex_expert"
    description = """Useful for when you need to answer questions related to crypto financial indicators on decentralized exchanges (DEX)"""
                ...
                json.dumps(
                    {
                        ...
                        "message": f"Using crypto dex tool...",
                    }
                ),
